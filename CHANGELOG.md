...
- **Heredoc and OpenAI-Ingress Replay Fixtures (M7.3):** fixtures now cover heredoc repair in bash blocks and DSML calls, and the replay harness can drive `/v1/chat/completions` (`"ingress": "openai"`) in JSON and streaming mode.
...
### Changed
- **Large Requests Are No Longer Cached:** requests whose canonical form exceeds `cache.max_prompt_bytes` (`LILTOK_CACHE_MAX_PROMPT_BYTES`, default 256 KB, 0 = no limit) skip cache lookup and storage. These are agent session transcripts that essentially never repeat; caching them only grew the database.
...
### Fixed
- **OpenAI-Format Clients Got Unrepaired Replies:** routed replies on `/v1/chat/completions` were sent as the provider's raw response, from before the router's repairs: tool calls written as text (DSML, `<tool_call>`) arrived as markup with `finish_reason: stop`, repaired tool names and recovered calls were lost, and a reply served by Anthropic arrived in Anthropic's format. The reply is now built from the router's result.
- **OpenAI Streaming Dropped Tool Calls:** streamed OpenAI-format replies carried only the text and always ended with `finish_reason: stop`, so structured tool calls never reached streaming clients. The stream now includes reasoning, tool calls (with index, id, name and arguments) and the real finish reason.
